...
void main() {
    vec2 uv = (gl_FragCoord.xy - u_res * 0.5) / (u_res * 0.5 * u_zoom) + u_center;
    vec2 z = uv;
    int iter = u_maxIter;
...
const uRes = gl.getUnifromLocation(prog, 'u_res')
const uCenter = gl.getUnifromLocation(prog, 'u_center')
const uZoom = gl.getUnifromLocation(prog, 'u_zoom')
const uC = gl.getUnifromLocation(prog, 'u_c')
const uMI = gl.getUnifromLocation(prog, 'u_maxIter')
...
function updateInfo() {
    ...
    document.getElementById('inf-zoom').textContent = zoom.toFixed(2) + 'x'
